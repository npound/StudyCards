...
class indexTableViewController: UITableViewController {
            ...
            if sourceViewController.haschanged == true
            {
                for i in 0...indexCards.count-1{
                    if(sourceViewController.oldkey == indexCards[i].keyword && sourceViewController.oldphrase == indexCards[i].phrase)
                    {
                        ...
                        indexCards[i].changed = NSDate.init(timeIntervalSinceNow: 0)
    if indexCards.count > 1{ sortButton.isEnabled = true}
                    }
    ...
    {
        for i in 0...indexCards.count-2
        {
            for j in (i+1)...indexCards.count-1
            {
                
                if !(indexCards[i].changed?.isGreaterThanDate(dateToCompare: indexCards[j].changed!))!
                {
                    let temp = indexCards[i]
                    indexCards[i] = indexCards[j]
                    indexCards[j] = temp
                }
    ...
    private func SortLastCreated()
    {
        for i in 0...indexCards.count-2
        {
            for j in (i+1)...indexCards.count-1
            {
                ...
                if !(indexCards[i].created?.isGreaterThanDate(dateToCompare: indexCards[j].created!))!
                {
                    let temp = indexCards[i]
                    indexCards[i] = indexCards[j]
                    indexCards[j] = temp
    ...
    private func SortAZHigh()
    {
        for i in 0...indexCards.count-2
        {
            for j in (i+1)...indexCards.count-1
            {
                if (!indexCards[i].keyword!.isGreater(stringCompared: indexCards[j].keyword!))
                {
                    let temp = indexCards[i]
                    indexCards[i] = indexCards[j]
                    indexCards[j] = temp
    ...
    override func viewDidLoad() {
        ...
        filterLabel.text = "Sort: High to Low"
        if indexCards.count > 1{ SortAZHigh()}
